...
Stay competitive in tech. Access this comprehensive catalog of cutting-edge skills and knowledge.
Mozilla ads
...
This page was last modified on May 14, 2023 by MDN contributors.
...
const lowerCase = (char) => result.toLowerCase();

function instanceCounter(counts, char) {
        counts[char] ? counts[char] + 1 : (counts[char] = 1);
}

const result = text.split('').filter(isValidChar).map(lowerCase).reduce(instanceCounter, {});
